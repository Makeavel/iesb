...
def create_individual():
    return np.round(np.random.uniform(0, 1, GENE_LENGTH), 2)
...
def fitness(individual):
    ...
    # Aqui, vamos usar a soma dos valores dos genes como exemplo
    return np.sum(individual)

def selection(population, fitnesses):
    # Implemente o método de seleção de acordo com o problema
    # Aqui, vamos usar a seleção por roleta como exemplo
...
def mutate(individual):
    for i in range(GENE_LENGTH):
        if random.random() < MUTATION_RATE:
            individual[i] += round(random.uniform(-0.1, 0.1), 2)
            individual[i] = round(max(min(individual[i], 1), 0), 2)
...
for generation in range(100):
    ...
    print(f"Generation {generation+1}: Average Fitness: {avg_fitness:.2f}, Best Individual: {[np.round(gene, 2) for gene in best_individual]}")